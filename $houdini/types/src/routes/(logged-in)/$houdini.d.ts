...
type LayoutRouteId = RouteId | "/(logged-in)/admin" | "/(logged-in)/chat-demo" | "/(logged-in)/dashboard" | "/(logged-in)/dashboard/[investingEntityId]" | "/(logged-in)/documents" | "/(logged-in)/pipeline" | "/(logged-in)/review-deals" | "/(logged-in)/submit-deals"
