...
type LayoutRouteId = RouteId | "/(auth)/forgot-password" | "/(auth)/log-in" | "/(auth)/log-out" | "/(auth)/new-password" | "/(auth)/sign-up" | "/(logged-in)/(dashboardWrapper)/dashboard" | "/(logged-in)/(dashboardWrapper)/review-deals" | "/(logged-in)/admin" | "/(logged-in)/admin/manage-fields" | "/(logged-in)/admin/manage-investing-entities" | "/(logged-in)/chat-demo" | "/(logged-in)/investments/[companyId]" | "/(logged-in)/investments/[companyId]/[documentCollectionId]" | "/(logged-in)/olddashboard" | "/(logged-in)/olddashboard/[investingEntityId]" | "/(logged-in)/submit-deals" | "/(public)" | "/(public)/about" | "/(public)/privacy-policy" | null
type LayoutParams = RouteParams & { companyId?: string; documentCollectionId?: string; investingEntityId?: string }
